...
const AzkarElmasa = () => {
  ...
  const azkarApi = "https://ahegazy.github.io/muslimKit/json/azkar_massa.json";
    ...
    <section className="h-full">
      <div className="max-w-screen-xl mx-auto pt-5 pb-5">
        ...
        {azkarMorning.map((azkar, i) => (
          <div
            key={i}
            className="border-2 border-color2 text-border p-8 bg-primary bg-opacity-60 rounded-xl mb-5 mx-3 text-center leading-9">
            <h2 className="font-semibold text-white text-xl mb-4 leading-8">
              {azkar.zekr}
            </h2>
            <p className="text-gray-300 text-xl">{azkar.bless}</p>
            <span className="bg-bgBtn border border-color2 px-10 pt-2 pb-2 mt-5 rounded-lg space-x-4 text-xl font-bold inline-block text-white">
              {azkar.repeat} مرات
            </span>
          </div>
        ))}
        ...
        <div className="flex justify-between select-none items-center gap-5 p-3 mt-10">
          ...
          <Link
            ...
            className="bg-primary bg-opacity-30 rounded-md p-2 max-sm:text-sm hover:bg-opacity-50 cursor-pointer text-gray-300 hover:underline hover:text-white">
            <span>
              الانتقال الى اذكار بعد انتهاء الصلآه
